document.addEventListener("DOMContentLoaded", () => {
    ...
    const cruciformContainer = document.querySelector(".cruciform-container");
    ...
    function updateCardCounts() {
        HANDS.forEach((hand) => {
            ...
            if (mobilePreview) {
                ...
                const cardString = Object.keys(SUITS).map(suit => {
                    const selectedRanks = Array.from(
                        handContainer.querySelectorAll(`.card-rank.selected[data-suit="${suit}"]`)
                    );
                    if (selectedRanks.length === 0) return `<div><span class="${SUIT_COLORS[suit]}">${SUITS[suit]}</span> -</div>`;
                    selectedRanks.sort((a, b) => RANKS.indexOf(a.dataset.rank) - RANKS.indexOf(b.dataset.rank));
    ...
    function attemptAutoComplete() {
        ...
        const incompleteHand = HANDS.find(hand =>
            document.getElementById(`${hand}-container`).querySelectorAll(".card-rank.selected").length < 13
        );
        if (!incompleteHand) return;

        const incompleteHandContainer = document.getElementById(`${incompleteHand}-container`);
        const unselectedInHand = incompleteHandContainer.querySelectorAll(".card-rank:not(.selected)");
    ...
    function runAnalysis() {
        const pbn = pbnInputDesktop.value.trim(); // Both inputs are synced, so either is fine
        if (!pbn) {
            showError("PBN入力が空です。");
            return;
        }
    ...
    pbnInputDesktop.addEventListener("input", () => syncPbnInputs(pbnInputDesktop, pbnInputMobile));
    ...
    // --- UI Helper Functions ---
    function displayResults(tricks) {
